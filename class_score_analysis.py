...
def analyze_data(data_1d):
    ...
    var = sum(x - mean for x in data_1d) ** 2 / len(data_1d)
    median = 0
    len = len(data_1d)
    for idx, x in enmerate(data_1d):
        if x > data_1d(idx + 1):
            data_1d[idx], data_1d[idx + 1] = data_1d[idx + 1], data_1d[idx]
    if len % 2 == 1:
        median = data_1d[(len - 1) / 2]
    else:
        median = (data_1d(len / 2) + data_1d(len / 2 - 1)) / 2
